...
- Краткое описание БТ и ссылки на детальные документы с бизнес-требованиями
  ...
  - Сервис должен поддерживать 8 основных функций:
...
- supervisor для выбора инструментов(сабагентов), которые нужны для ответа на вопрос
...
Всего в данной задаче 2 вещи, которые подразумевают в себе ML - модуль с цифровым аватаром и модуль с LLM агентом. Модуль с LLM агентом в свою очередь содержит две компоненты, которые используют ML - RAG и суммаризация статьи. Все эти компоненты лучше рассмотреть по отдельности:
...
Предполагает оценку пользователя ответа от компоненты. Это будет использоваться для дальнейшей оценки работы компоненты и улучшения ее качества на основе фидбека пользователя.
